...
class News_FrontController extends Cible_Controller_Action {
    ...
    public function init() {
        ...
        $this->_obj = new NewsManagerObject();
    ...
    public function listAction()
    {
        ...
        'NI_Title' => array('width' => '350px', 'label' => $this->view->getCibleText('list_column_id')),
        'S_Code' => array('width' => '100px',
            'postProcess' => array(
                'type' => 'dictionnary',
                'prefix' => 'status_'
            )),
        ...
        $this->_joinTables = array(
            array('obj' => 'TeamsObject', 'foreignKey' => 'TED_NewsCatId = ND_CategoryID', 'dataOnly' => true),
            array('obj' => 'CategoriesObject', 'foreignKey' => 'ND_CategoryID')
            );
        $this->_obj->setOrderBy('ND_Date DESC');
        $tmp = $this->_obj->getIndexColumns();
        $this->_obj->setIndexSelectColumns(array_combine(array_keys(array_flip($tmp)), $tmp));
        $this->_formName = 'FormNews';
    ...
    public function addAction()
    {
        ...
        $form->populate(array('ND_CategoryID' => $this->_obj->getBlockParams(1)));
        if ($this->_request->isPost())
        {
            ...
            {
                if (empty($formData['NI_Status'])){
                    $formData['NI_Status'] = 2;
                }
    ...
    public function editAction() {
        ...
        $catId = (int) $this->_getParam('category');
        $baseDir = $this->view->baseUrl() . "/";
        $returnAction = $this->_getParam('return');
        ...
        if (empty($data) || $data['ND_CategoryID'] != $catId){
        ...
        elseif($data['ND_CategoryID'] == $catId)
        {
            $formData = $this->_request->getPost();
...
//        $this->_tables[] = $oData->getIndexTableName();
        ...
        if ($this->_actionKey == 'list'){
            $select->joinLeft('Status', 'NI_Status = S_ID');
        }
        if ($this->_obj->getBlockParams(1) > 0){
            $select->where('ND_CategoryID = ?', $this->_obj->getBlockParams(1));
        }
...
//                $this->_tables[] = $tmpIndexTable;
    ...
    private function _setActionsList($tabId, $page = 1)
    {
        ...
        if (count($this->_actionsList) == 0)
        {
            ...
            $options['renderPartial'] =  'partials/generic.front.list.phtml';
